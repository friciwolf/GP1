...
\begin{document}
	...
	mit der Ordnung n.
		...
		\begin{figure}
		\begin{center}
			\includegraphics[scale=0.9]{Images/stehende_welle.png}
			\caption{Schematische Darstellung einer stehenden Welle im Rohr (Quelle: Demtröder: Experimentalphysik 1)}
			\label{welle_stehend}
		\end{center}
	\end{figure}
	...
	\begin{equation}
		...
		\label{Kalib}
	\end{equation}
	mit einem $\chi^2/f \approx 2.704/4 \approx 0.676$. Der Residuenplot dazu ist in Abb.\ref{Kalib_Res} dargestellt. Man erkennt, dass alle Fehler $\sqrt{k^2*R_{err}^2+L_{err}^2}$ etwa im Rahmen einer Standardabweichung um die Funktion gestreut sind.\\   
...
Aus dem y-Achsenabschnitt kann man auch die Postion der Schallquelle ablesen, die nach der obigen, im Abschnitt "Kalibrierung des Wegaufnehmers" getroffenen Wahl des Koordinatensystems $(21.556 \pm 0.058)cm$ von der Position $L=0$ entfernt ist.
	...
	wobei der statistische Fehler mit dem Gaußschen Fehlerfortpflanzung und der Systematische (wofür auch eine Kalibrierungsunsicherheit des Messbands von $\pm$0.7mm angenommen wurde) mithilfe der Verschiebemethode berechnet wurden. Für die Fortpflanzung wurde die Gleichung $\sigma_v=2La\sqrt{(\sigma_a/a)^2+(\sigma_L/L)^2}$ verwendet. Dieses Ergebnis liegt aber deutlich unterhalb des theoretischen Werts, was sowohl an dem möglichen fehlerhaften Aufbau des Experiments (z.B. Mikrofonende lag innerhalb des Rohrs), als auch an den zu großen Unsicherheiten und Ungenauigkeiten in der erhaltenen Datenmenge zurückgeführt werden könnte. Auch die Möglichkeit, bei der Datenaufzeichnung einen unbekannten Fehler gemacht zu haben, ist nicht ausgeschlossen.
