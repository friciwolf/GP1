...
plt.plot(np.arange(0.5, 6.5, 0.01), a*np.arange(0.5, 6.5, 0.01)+b, label="f = ("+str(np.round(a,2))+"$\pm {}$) Hz*n+(".format(np.round(ea,2))+str(np.round(b,2))+"$\pm {}$)Hz \n $Chi^2 /f = {}/{}  ≈ {}$".format(np.round(eb,2),np.round(chiq,2),2,np.round(chiq/2,2)))
