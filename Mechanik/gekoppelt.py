...
for i in range(1,5):
    ...
    for j in range(0,file_count):
        ...
        plt.savefig("Auswertung/Messung "+str(i)+"_"+name[j]+".jpg")
        '''
        count=0
        for i,volt in enumerate(M):
    
            if volt!=U1[j][-1] and ((volt<=0 and U1[j][i+1]>0) or (volt>=0 and U1[j][i+1]<0)):
        ...
        errt=(t[j][-1]-t[j][0])/len(t[j])
    
        T = 2*(t[j][ende]-t[j][start])/count
    
        errT = 2*np.sqrt(2)*errt/count #TODO: Richtig?
        ...
        plt.savefig("Auswertung/Messung "+str(i)+"_"+name[j]+"Fourier.jpg")
        ...
        count=0
        '''
        for k,volt in enumerate(U1[j]):
    
            if volt!=U1[j][-1] and ((volt<=0 and U1[j][k+1]>0) or (volt>=0 and U1[j][k+1]<0)):
