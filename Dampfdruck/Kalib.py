...
#Gasdichtigkeit
    ...
    print('Verlust: ({} +-{}) mbar/s bei einem Innendruck von etwa {}+-{} mbar\n'.format(m,em,mp,errp)) #so genehmigt
...
#Kalibrierung
    ...
    T=T[2000:]
    mT0,sT0,errT0=Rauschmessung.round_good(np.mean(T),np.std(T),np.std(T)/np.sqrt(len(T)))
    data2 = cassy.CassyDaten("Temperatur, Druck bei 100.lab")
    t2 = data2.messung(1).datenreihe("t").werte
    p2 = data2.messung(1).datenreihe("p_B1").werte
    T2 = data2.messung(1).datenreihe("&J_A11").werte
    T_sliced = T2[240:1500]
    mT100,sT100,errT100=Rauschmessung.round_good(np.mean(T_sliced),np.std(T_sliced),np.std(T_sliced)/np.sqrt(len(T_sliced)))
    
    a,ea,b,eb,chiq,corr = ana.lineare_regression(np.array([0, 100])-50,np.array([mT0, mT100])-50, np.array([Rauschmessung.sT,Rauschmessung.sT])) 
    asys,easys,bsys,ebsys,chiq,corr = ana.lineare_regression(np.array([0, 100])-50,np.array([mT0, mT100])-50, np.array([sigmaT_Gerät_kl_70,sigmaT_Gerät_gr_70]))
    a,ea,easys=Rauschmessung.round_good(a,ea,easys)
    b,eb,ebsys=Rauschmessung.round_good(b,eb,ebsys)
