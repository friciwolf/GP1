...
plt.plot(t, p)
m,em,b,eb,chiq,corr = anal.lineare_regression_xy(t, p, sigmat*np.ones(len(t)), sigmap*np.ones(len(p)))
plt.plot(t, m*t+b, color='green')
plt.title("Dichtigkeitsmessung aka Druncksanstieg im evakuierten Kolben")
plt.show()
print('m = (%g +- %g) Pa/s,   b = (%g +- %g) Pa,  chi2/dof = %g / %g  corr = %g' % (m, em, b, eb, chiq, len(t)-2, corr))
...
plt.title("Streuung der gemessten Temperaturwerte bei T=100°C")
plt.show() #=> merkwürdig... sehr merkwürdig, die hier ist nicht gaussförmig genug...
