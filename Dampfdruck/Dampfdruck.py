...
for i in range(1,11):
    ...
    Lsys,eLsys,bsys,ebsys,chiq,corr = ana.lineare_regression_xy(xi, yi, np.ones(len(xi)), np.ones(len(yi)))
    L_Werte[0].append(L)
    L_Werte[1].append(eL)
    L_Werte[2].append(eLsys)
    T_Werte[0].append(T_real[1000*i])
    T_Werte[1].append(sTstat[1000*i])
    T_Werte[2].append(sTsys[1000*i])#TODO: Fehler
...
plt.savefig('Images/Dampfdruck_')
plt.figure()
